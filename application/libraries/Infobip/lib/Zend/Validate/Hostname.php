...
class Zend_Validate_Hostname extends Zend_Validate_Abstract
{
    ...
    public function isValid($value)
    {
        ...
        if ((count($domainParts) > 1) && (strlen($value) >= 4) && (strlen($value) <= 254)) {
            ...
            $origenc = iconv_get_encoding(ini_get('default_encoding'));
            iconv_set_encoding(ini_get('default_encoding'), 'UTF-8');
            do {
                // First check TLD
                $matches = array();
            ...
            iconv_set_encoding(ini_get('default_encoding'), $origenc);
